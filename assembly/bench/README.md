...
3. **Full stack (`stack`), one channel at a time:** power + digital + left channel (input shorted) → rails at power J3, then the left modulator output QL at digital J4.12 against MCLK; add the right channel → rails again, QR at J4.14; then references, PI_BCLK/PI_LRCLK and finally PI_DIN/PI_BCLK at the Pi header: PI_DIN must toggle continuously at 3.3 V logic. No Pi, +3.3 V still from AD3 V+. A pass here is the gate for connecting the Pi, which then supplies the 3.3 V (remove AD3 V+ first). Individual board results do not prove the full-stack power margin.
...
5. **Audio performance (`audio`):** the finished converter as it is used, stack powered from the Pi and the ripper running. Only W1 and AD3 GND go to the channel input J20; the ripper is asked to hold recording and returns its decimated 48 kHz audio for each step (`/api/audio`). The script calibrates full scale from a 0.5 Vpk tone, then records idle noise (unweighted and A-weighted, hum at 50/100/150/200 Hz, SNR), 1 kHz at −3…−60 dBFS (THD, THD+N, 2nd/3rd harmonic, AES17 dynamic range from the −60 dBFS tone), crosstalk, a third-octave response at −20 dBFS and CCIF 19+20 kHz and SMPTE 60 Hz + 7 kHz intermodulation. Figures are recorded, not judged. The run folder gets spectrum and response PNGs and a `summary.md` table for the write-up. The AD3's 14-bit generator bounds distortion and noise figures near −80 dB; anything better than that is the generator. `--channel right` measures the other input; `--pi` names the ripper. Run it once before and once after each hardware change to see what the change bought.
...
No AD3 was attached when these scripts were created; on 16 September 2026 the `devices` command enumerated the AD3 through the installed SDK on this computer. Real acquisition, wiring, analog acceptance limits and board behaviour still need the first physical run, which is the next step.
