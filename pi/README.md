...
- **Where the side starts.** The identified track is an *anchor* (which track starts at which second of the side): tracks before it are counted back from there, tracks after it forward, so a needle dropped mid-side is handled. Tracks that were not played whole (dropped into, or lifted from) are marked incomplete and are neither counted nor encoded.
