...
[Assembly guide & bench log](https://vinyl-adc.madsrudolph.dev/) · [Bring-up log, 16–18 September](docs/bringup-log-2026-09.md) · [16 September bench plan](assembly/sessions/2026-09-16/README.md) · [6 September test handoff](assembly/sessions/2026-09-06/README.md)
...
**Rev D (22 September):** with PCBWay sponsoring the fabrication, the one-board converter is redrawn as a proper [Raspberry Pi HAT](hardware/kicad/rev_d/README.md) — 140 × 136 mm (its layout redone on 24 September: every passive beside the pin it serves, channel R a copy of channel L), four layers, still every part through-hole from the DTU shop — and given the things the bench asked for: an analog supply island behind its own choke (the DAC reference rail, kept away from the Pi's and the charge pump's switching), a second 74HC244 to stiffen the −5 V pump, a damped MCLK, eight front-edge status LEDs (rail, pump, clock-alive, clip L/R from LM339 window comparators, and three under the Pi's control), shutdown and user buttons, a HAT ID EEPROM, test-point headers with ground pins, and a tonearm ground terminal with a lift network. `tools/check_rev_d.py` proves it is the reference circuit plus exactly those additions; [production files](production/vinyl_adc_rev_d/) and the [bill of materials](docs/bom-rev-d.md) are exported. Design record: [design-notes §12](docs/design-notes.md#12-rev-d-the-converter-as-a-raspberry-pi-hat-22-september-2026).

**Rev E (24 September):** the same circuit in common [surface-mount parts](hardware/kicad/rev_e/README.md) on a 118 × 88 mm four-layer HAT — 0805 C0G/X7R and thin-film parts, SOIC logic and op-amps, SOT-23 transistors, shielded SMD chokes — checked by the same gate as rev D, placed and routed by the same tools, DRC clean. [Production files](production/vinyl_adc_rev_e/) include the pick-and-place file for PCBWay assembly; [bill of materials](docs/bom-rev-e.md); design record [design-notes §13](docs/design-notes.md).
...
| **Master Clock ($F_{clk}$)** | **6.144 MHz crystal Pierce oscillator** (74HCU04 + HC-49 crystal) | Direct low-jitter clock ($<50\text{ ps}$ jitter vs. Pi PLL phase noise); also available as a plug-in module for the rev A board |
